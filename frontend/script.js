...
class LoanPredictionSystem {
    ...
    async loadModelInfo() {
        try {
            ...
            if (response.ok) {
                ...
                // Update hero stats - Model Accuracy
                if (modelInfo.training_metrics?.accuracy) {
                    const accuracyElement = document.getElementById('model-accuracy');
                    if (accuracyElement) {
                        accuracyElement.textContent = `${(modelInfo.training_metrics.accuracy * 100).toFixed(1)}%`;
                    }
                }

                // Update model info section
                const modelTypeElement = document.getElementById('modelType');
                if (modelTypeElement && modelInfo.model_type) {
                    modelTypeElement.textContent = modelInfo.model_type.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase());
                }

                const modelAccuracyElement = document.getElementById('modelAccuracy');
                if (modelAccuracyElement && modelInfo.training_metrics?.accuracy) {
                    modelAccuracyElement.textContent = `${(modelInfo.training_metrics.accuracy * 100).toFixed(1)}%`;
                }

                const featureCountElement = document.getElementById('featureCount');
                if (featureCountElement && modelInfo.feature_count) {
                    featureCountElement.textContent = modelInfo.feature_count;
                }

                // Update analytics section
                this.updateAnalyticsSection(modelInfo);

                console.log('[SUCCESS] Model info loaded and UI updated');
            } else {
                console.warn('Failed to load model info - API response not OK');
                this.handleModelInfoError();
            }
        } catch (error) {
    ...
    /**
     * Handle model info loading errors
     */
    handleModelInfoError() {
...
Generated by Loan Status Prediction - Professional Loan Prediction System
        `.trim();
    }
